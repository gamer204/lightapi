module.exports = function parseRoute (route, routes) {
	...
	if(typeof routes[route] === "string") {
		...
		method = ctrl[1];
		ctrl = ctrl[0];
		if(_.contains(["get", "post", "put", "delete"], (route.match(/GET|POST|PUT|DELETE/i) || "").toLowerCase())) {
			route = route.match(/\/[a-zA-Z0-9\/]+$/);
			verb = route.match(/GET|POST|PUT|DELETE/i);
			log.log('route.match(/GET|POST|PUT|DELETE/i)');
		}
