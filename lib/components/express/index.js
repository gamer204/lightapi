...
app.configure(function () {
	...
	app.use(express.session({ secret: la.config.local.secret || Math.random().toString(36), key: "lightapi.sid" }));
	app.use(express.csrf());
	if(typeof la.config.middlewares === "function")
		la.config.middlewares(app);
});
